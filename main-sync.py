            ...
            if not moving:
                ...
                if self.game_mode == "CHALLENGE":
                    if self.stones_thrown[0] == 1: 
                        ...
                        elif self.c_type == "TAKEOUT": self.challenge_success = (self.challenge_takeout_target not in self.stones) and any(s.team == 0 and (pygame.math.Vector2(s.pos) - pygame.math.Vector2(cx, cy)).length() <= 252 for s in self.stones)
                    ...
                    if self.stones_thrown[0] >= self.stones_per_team and self.stones_thrown[1] >= self.stones_per_team:
                        in_house = [((s.pos - self.house_pos).length(), s.team) for s in self.stones if (s.pos - self.house_pos).length() <= 252]
                        if in_house:
