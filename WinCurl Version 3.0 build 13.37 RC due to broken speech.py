...
def trigger_vibration(duration_s):
    if HAS_HAPTICS:
        try:
            vibrator.vibrate(time=duration_s)
        except:
            pass
...
def draw_maple_leaf(surface, cx, cy, scale, color):
    """Draws a stylized maple leaf for the Canadian context."""
    points = [
...
def draw_hammer_icon(surface, x, y, color):
    """Draws a small hammer icon for UI indicators."""
    pygame.draw.rect(surface, color, (x, y, 16, 8), border_radius=2)
    pygame.draw.rect(surface, color, (x+6, y+8, 4, 12))

class UICache:
    """Caches generated UI surfaces (like glass buttons) to avoid redrawing them every frame."""
    glass_surfs = {}
...
def draw_glass_rect(surface, rect, base_color, border_radius=16, is_hovered=False):
    """Draws a premium glass-styled rectangle using the cache."""
...
class WinCurlAudioEngine:
    """Procedurally generates all sound effects using math to avoid external dependencies."""
    def __init__(self):
        ...
        self.snd_sweep = self._synthesize_sweep()
        self.snd_throw = self._synthesize_throw()
        self.snd_clack = self._synthesize_clack()
        ...
        self.snd_chal_comp = self._synthesize_vosim_phrase("CHALLENGE", 1.5)
        self.snd_red_wins = self._synthesize_vosim_phrase("RED", 1.2)
        self.snd_ylw_wins = self._synthesize_vosim_phrase("YELLOW", 1.2)
    ...
    def play_hover(self): 
        ...
        trigger_vibration(0.02)
        
    def play_click(self): 
        self.ch_ui.play(self.snd_click)
        trigger_vibration(0.04)
...
class Starfield:
    """Renders a simple moving starfield background in menus."""
    def __init__(self):
        self.stars = [(random.uniform(0, 1), random.uniform(0, 1), random.uniform(0.5, 3.0)) for _ in range(150)]
        
    def draw(self, surface, speed_mult=1.0):
        w, h = surface.get_size()
        for i in range(len(self.stars)):
            x_pct, y_pct, s = self.stars[i]
            y_pct = (y_pct + (s * speed_mult) / h) % 1.0
            self.stars[i] = (x_pct, y_pct, s)
            c = int(min(255, 30 + s*60))
            pygame.draw.circle(surface, (c, c, c), (int(x_pct * w), int(y_pct * h)), max(1, int(s)))
...
# --- Game Entities ---
class Stone:
    # OPTIMIZATION: Cache full un-rotated bases to save thousands of Pygame calls per second.
...
class AnimatedCurler:
    """Manages the visual representation of the throwing curler."""
...
class WinCurl3:
    ...
    def preload_assets(self):
        ...
        # Fractal Menu Pre-render optimization
        self.fractal_surf = pygame.Surface((1000, 1000), pygame.SRCALPHA).convert_alpha()
        self.draw_fractal_house(self.fractal_surf, 500, 500, 220, 0, 3, 0)
    ...
    def setup_display(self):
        ...
        self.btn_chat = pygame.Rect(30, 140, 120, 50)

        self.menu_buttons = [
            {"id": "local", "y": 480, "text": "Local 1v1", "color": HOUSE_RED, "scale": 1.0},
            {"id": "bot", "y": 620, "text": "Local vs Bot", "color": TEAM_YELLOW, "scale": 1.0},
            {"id": "chal", "y": 760, "text": "Challenge Mode", "color": PURPLE_SUIT, "scale": 1.0},
            {"id": "color", "y": 900, "text": "My Team:", "color": HOUSE_RED, "scale": 1.0},
            {"id": "name", "y": 1040, "text": f"Name: {self.username}", "color": (130, 140, 155), "scale": 1.0},
            {"id": "host", "y": 1180, "text": "Host IRC", "color": HOUSE_BLUE, "scale": 1.0},
            {"id": "join", "y": 1320, "text": "Join IRC", "color": HOUSE_BLUE, "scale": 1.0},
            {"id": "exit", "y": 1460, "text": "Exit Game", "color": HOUSE_RED, "scale": 1.0}
        ]
                ...
                if 0 < dist < min_dist:
                    ...
                    if v_normal > 0:
                        ...
                        if impulse * 12 > 0.8: 
                            ...
                            trigger_vibration(min(0.1, impulse * 0.02))
                            mid_x, mid_y = (s1.pos.x + s2.pos.x) / 2, (s1.pos.y + s2.pos.y) / 2
                            for _ in range(int(impulse * 5)): 
                                self.particles.append({'pos': pygame.math.Vector2(mid_x, mid_y), 'vel': normal.rotate(random.uniform(-45, 45)) * random.uniform(2, 10), 'life': 1.0, 'decay': random.uniform(0.02, 0.05), 'type': 'spark'})
    ...
    def fire_stone(self):
        ...
        if pull.length() > 5:
            ...
            self.curler_anim.update("LUNGING"); self.audio.play_throw(); trigger_vibration(0.08)
    ...
    def handle_play_events(self, event):
        ...
        if self.typing_target == "chat":
            if event.type == KEYDOWN:
                if event.key == K_RETURN:
                    if self.chat_input:
                        self.net.send_action({'cmd': 'chat', 'msg': self.chat_input})
                        self.chat_history.append(f"Me: {self.chat_input}")
                        self.chat_input = ""
                    self.set_typing_target(None)
                elif event.key == K_ESCAPE:
                    self.set_typing_target(None)
                elif event.key == K_BACKSPACE: self.chat_input = self.chat_input[:-1]
                elif event.unicode.isprintable() and len(self.chat_input) < 40: self.chat_input += event.unicode
            return
    ...
    def update_physics(self):
        ...
        if self.turn_state == "SLIDING":
            ...
            if is_sweeping and delta > 4:
                ...
                self.broom_pos = mouse_pos
                self.sweep_active = True
                for _ in range(2 if IS_ANDROID else 3): 
                    self.particles.append({'pos': mouse_pos + pygame.math.Vector2(random.uniform(-30, 30), random.uniform(-30, 30)), 'vel': pygame.math.Vector2(random.uniform(-1, 1), random.uniform(-3, 0)), 'life': 1.0, 'decay': random.uniform(0.02, 0.05), 'type': 'sweep'})
            else:
                self.sweep_active = False
            ...
            for s in self.stones:
                ...
                if s.is_moving and s.vel.length() > 0.5: 
                    self.particles.append({'pos': s.pos + pygame.math.Vector2(random.uniform(-15, 15), random.uniform(-15, 15)), 'vel': s.vel * -0.1, 'life': 1.0, 'decay': random.uniform(0.01, 0.03), 'type': 'trail'})
            
            if self.game_mode in ["HOST", "JOIN"] and pygame.time.get_ticks() % 3 == 0: 
            ...
            for s in self.stones:
                if s.is_moving:
                    if s.pos.x - s.radius < 0 or s.pos.x + s.radius > BASE_WIDTH or s.pos.y < -s.radius:
                        ...
                        for _ in range(8): 
                            self.particles.append({'pos': pygame.math.Vector2(s.pos), 'vel': pygame.math.Vector2(random.uniform(-4, 4), random.uniform(-4, 4)), 'life': 1.0, 'decay': 0.05, 'type': 'spark'})
                    else:
                        moving = True
                ...
                if self.game_mode == "CHALLENGE":
                    if self.stones_thrown[0] == 1: 
                        ...
                        if self.c_type in ["DRAW", "GUARD"]: 
                            ...
                            self.challenge_success = (self.challenge_takeout_target not in self.stones) and any(s.team == 0 and s.pos.y < cy + 210 for s in self.stones)
                        elif self.c_type == "DOUBLE": 
                            self.challenge_success = len([s for s in self.stones if s.team == 1]) == 0 and any(s.team == 0 for s in self.stones)
                        self.turn_state = "END"
                else:
        ...
        elif self.turn_state == "AIMING" and self.game_mode == "BOT" and self.current_team != self.preferred_color: 
            self.execute_ai()
    ...
    def update_network(self):
        ...
        if data:
            ...
            elif data.get('cmd') == 'chat':
                self.chat_history.append(f"Opp: {data['msg']}")
                if len(self.chat_history) > 6: self.chat_history.pop(0)
                self.audio.play_hover()
        ...
        if self.game_mode == "HOST" and self.app_state == "COIN_TOSS" and self.coin_timer == 50: 
    ...
    def draw_fractal_house(self, surf, x, y, radius, depth, max_depth, morph_time):
        ...
        pygame.draw.circle(surf, color, (int(x), int(y)), int(radius))
        pygame.draw.circle(surf, color, (int(x), int(y)), int(radius), max(1, 6 - depth))
        for i in range(3):
    ...
    def draw_menu(self):
        self.canvas.fill((10, 12, 16, 0)) # Fully transparent fill to let stars show through on desktop
        ...
        # Pre-rendered Title Graphic processing
        bx, by = cx - self.title_base.get_width()//2, 80 + int(math.sin(t_ms * 4.0) * 15) 
        self.canvas.blit(self.title_shadow, (bx-5, by-5))
        
        grad_surf = pygame.Surface(self.title_base.get_size(), pygame.SRCALPHA).convert_alpha()
        offset = int(t_ms * 100) % 500
        grad_surf.blit(self.rainbow_grad, (-offset, 0))
        grad_surf.blit(self.title_base, (0, 0), special_flags=pygame.BLEND_RGBA_MULT)
    ...
    def draw_ui(self):
        ...
        # New Feature: Draw Broom Visual while Sweeping
        if getattr(self, 'sweep_active', False) and self.broom_pos is not None:
            bx, by = self.broom_pos.x, self.broom_pos.y
            offset = math.sin(pygame.time.get_ticks() * 0.04) * 18  # Sweeping back and forth
            # Broom Head
            pygame.draw.rect(self.canvas, (210, 180, 120), (bx - 25 + offset, by - 8, 50, 16), border_radius=4)
            # Broom Handle (going off screen)
            pygame.draw.polygon(self.canvas, (90, 70, 40), [
                (bx - 4 + offset, by - 8), (bx + 4 + offset, by - 8),
                (bx + 15 + offset, by - 120), (bx + 5 + offset, by - 120)
            ])

        # Multiplayer Chat UI
        if self.game_mode in ["HOST", "JOIN"]:
            draw_glass_rect(self.canvas, self.btn_chat, HOUSE_BLUE, self.btn_chat.h//2, self.btn_chat.collidepoint(m_pos.x, m_pos.y))
            lbl_c = self.small_font.render("CHAT", True, WHITE)
            self.canvas.blit(lbl_c, lbl_c.get_rect(center=self.btn_chat.center))

            y_offset = 220
            for msg in self.chat_history:
                txt = self.small_font.render(msg, True, WHITE)
                self.canvas.blit(txt, (30, y_offset))
                y_offset += 35

            if self.typing_target == "chat":
                pygame.draw.rect(self.canvas, (0, 0, 0, 150), (30, y_offset, 600, 40))
                txt = self.small_font.render(self.chat_input + "_", True, TEAM_YELLOW)
                self.canvas.blit(txt, (35, y_offset + 5))

        # Pause Button Updates
        draw_glass_rect(self.canvas, self.btn_pause, (200, 210, 230), self.btn_pause.h // 2, self.btn_pause.collidepoint(m_pos.x, m_pos.y))
        lbl_p = self.small_font.render("|| PAUSE", True, BLACK); self.canvas.blit(lbl_p, lbl_p.get_rect(center=self.btn_pause.center))
    ...
    def render(self):
        ...
        # Fullscreen Starfield Injection
        if self.app_state in ["MENU", "CHALLENGE_MENU", "PAUSED"]:
            self.starfield.draw(self.screen, 2.0 if self.app_state != "PAUSED" else 0.5)
...
class IRCNetworkManager:
    """Manages matchmaking and basic multiplayer sync via an IRC channel."""
    def __init__(self):
        if IS_ANDROID: pygame.mixer.pre_init(44100, -16, 2, 4096)
        else: pygame.mixer.pre_init(44100, -16, 2, 1024)
        pygame.mixer.init()
