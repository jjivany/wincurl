...
print("\n" + "="*60)
print("     [SYSTEM] WINCURL 3 BUILD 12 (PUBLIC RELEASE)")
...
class ThreeDStone:
    def draw(self, surface, center_x, center_y, mouse_pos):
        ...
        pygame.draw.circle(surface, (60, 65, 70), (int(bx), int(by)), 80)
        ...
        for i in range(120):
            ...
            pygame.draw.circle(surface, h_col, (int(px), int(py - math.sin(p*math.pi)*45)), 16) 
            pygame.draw.circle(surface, HOUSE_RED, (int(px), int(py - math.sin(p*math.pi)*45)), 12) 
...
class Stone:
    ...
    def draw(self, surface):
        ...
        pygame.draw.circle(surface, (160, 165, 170), (int(self.pos.x), int(self.pos.y)), self.radius)
        pygame.draw.circle(surface, (100, 105, 110), (int(self.pos.x), int(self.pos.y)), self.radius, 3)
        pygame.draw.circle(surface, (180, 185, 190), (int(self.pos.x), int(self.pos.y)), self.radius - 8)
        
        pygame.draw.circle(surface, color, (int(self.pos.x), int(self.pos.y)), 22)
        pygame.draw.circle(surface, (max(0, color[0]-50), max(0, color[1]-50), max(0, color[2]-50)), (int(self.pos.x), int(self.pos.y)), 22, 3)
...
class WinCurl3:
    def __init__(self):
        ...
        pygame.init(); self.screen = pygame.display.set_mode((600, 900), pygame.RESIZABLE); pygame.display.set_caption('"WinCurl" 3')
        self.canvas = pygame.Surface((BASE_WIDTH, BASE_HEIGHT)); self.clock = pygame.time.Clock()
        self.audio = WinCurlAudioEngine(); self.net = IRCNetworkManager()
        self.is_fullscreen = False
        self.dragging_slider = False
        
        self.save_file = os.path.join(os.path.expanduser("~"), ".wincurl3_save.json")
        self.load_progress()
        
        self.house_pos = pygame.math.Vector2(BASE_WIDTH // 2, 350); self.hack_pos = pygame.math.Vector2(BASE_WIDTH // 2, BASE_HEIGHT - 150)
        ...
        self.menu_stone = ThreeDStone(); self.particles = []; self.shake_amount = 0.0; self.pause_anim = 0.0
        ...
        self.btn_pause, self.btn_resume = pygame.Rect(BASE_WIDTH//2-100, BASE_HEIGHT-100, 200, 70), pygame.Rect(BASE_WIDTH//2-250, BASE_HEIGHT//2-100, 500, 100)
    ...
    def toggle_fullscreen(self):
        ...
        else: self.screen = pygame.display.set_mode((600, 900), pygame.RESIZABLE)

    def load_progress(self):
        try:
            with open(self.save_file, "r") as f:
                data = json.load(f)
                self.challenge_progress = data.get("challenge", [False] * 30)
            ...
            self.challenge_progress = [False] * 30; self.username = ""; self.preferred_color = 0; self.total_ends_pref = 8; self.room_text = ""; self.ai_difficulty = 5
    ...
    def save_progress(self):
        try:
            data = {"challenge": self.challenge_progress, "username": self.username, "color": self.preferred_color, "ends": self.total_ends_pref, "room": self.room_text, "bot_skill": self.ai_difficulty}
    ...
    def start_match(self):
        ...
        if self.game_mode == "CHALLENGE":
            ...
            self.app_state = "COIN_TOSS"; self.coin_timer = 180; self.coin_flip_result = random.choice([0, 1])
    ...
    def load_challenge(self, level):
        ...
        if level <= 5: 
            self.c_type = "DRAW"
            self.challenge_text_1, self.challenge_text_2 = f"[{level}/30] THE DRAW", "Land inside the highlighted target."
            self.challenge_target = (cx + (level%2)*30, cy + (5-level)*40, max(20, 80 - level*10))
        elif level <= 10: 
            self.c_type = "TAKEOUT"
            self.challenge_text_1, self.challenge_text_2 = f"[{level}/30] TAKEOUT", "Remove the yellow rock. Stay in play."
            s = Stone(cx + (level-7)*15, cy + (level-8)*20, 1); self.stones.append(s); self.challenge_takeout_target = s
            if level > 8: self.stones.append(Stone(cx - 30, cy + 180, 0)) 
        elif level <= 15: 
            self.c_type = "DRAW" 
            self.challenge_text_1, self.challenge_text_2 = f"[{level}/30] HIT AND ROLL", "Hit the rock and roll into the target."
            self.stones.append(Stone(cx + 40, cy + 50, 1)); self.challenge_target = (cx - 40, cy, 45)
        elif level <= 20: 
            self.c_type = "GUARD"
            self.challenge_text_1, self.challenge_text_2 = f"[{level}/30] THE GUARD", "Place a rock in the target zone (Guard)."
            self.challenge_target = (cx, cy + 300 - (level-16)*20, 40)
        elif level <= 25: 
            self.c_type = "DOUBLE"
            self.challenge_text_1, self.challenge_text_2 = f"[{level}/30] DOUBLE TAKEOUT", "Clear ALL yellow rocks from the house."
            self.stones.extend([Stone(cx - 25, cy + 15, 1), Stone(cx + 25, cy - 15, 1)])
            if level > 22: self.stones.append(Stone(cx, cy + 60, 1)) 
        else: 
            self.c_type = "TAKEOUT"
            self.challenge_text_1, self.challenge_text_2 = f"[{level}/30] ANGLE RAISE", "Raise the red guard into the yellow rock."
            self.stones.append(Stone(cx, cy + 150, 0)) 
            s = Stone(cx - 20, cy, 1); self.stones.append(s); self.challenge_takeout_target = s
            if level == 30: self.stones.append(Stone(cx + 40, cy + 100, 1)) 
    ...
    def fire_stone(self):
        ...
        if pull.length() > 10:
            self.active_stone.vel = pull.normalize() * min(42.0, pull.length() / 8.0)
    ...
    def handle_menu_events(self, event):
        ...
        if event.type == MOUSEBUTTONDOWN and event.button == 1:
            if 300 < mx < 900:
                for b in self.menu_buttons:
                    if b["y"] < my < b["y"] + 110 * b["scale"]:
                        self.audio.play_click(); self.typing_target = None
                        if b["id"] == "local": self.game_mode = "LOCAL"; self.audio.stop_music(); self.start_match()
                            ...
                            self.app_state = "ROOM_PROMPT"; self.typing_target = "room"; self.net_action = b["id"]
                        elif b["id"] == "name": self.typing_target = "name"
            ...
            if event.key == K_RETURN: self.typing_target = None; self.save_progress()
    ...
    def handle_room_prompt_events(self, event):
        if event.type == KEYDOWN and self.typing_target == "room":
            if event.key == K_RETURN and len(self.room_text) > 0:
                ...
                self.app_state = "MENU"; self.typing_target = None
    ...
    def update_physics(self):
        ...
        if self.turn_state == "SLIDING":
            ...
            if is_sweeping and delta > 4:
                ...
                for _ in range(3): self.particles.append({'pos': mouse_pos + pygame.math.Vector2(random.uniform(-30, 30), random.uniform(-30, 30)), 'vel': pygame.math.Vector2(random.uniform(-1, 1), random.uniform(-3, 0)), 'life': 1.0, 'decay': random.uniform(0.02, 0.05), 'type': 'sweep'})
            ...
            if not moving:
                ...
                self.stones = [s for s in self.stones if 0 < s.pos.x < BASE_WIDTH and s.pos.y > (self.house_pos.y - 150) and s.pos.y < (self.house_pos.y + 400 + s.radius)]
    ...
    def update_network(self):
        ...
        if self.app_state == "MENU" and self.net.matched:
            self.app_state = "COIN_TOSS"; self.coin_timer = 180; self.coin_flip_result = random.choice([0, 1]) if self.game_mode == "HOST" else -1
        ...
        if self.game_mode == "HOST" and self.app_state == "COIN_TOSS" and self.coin_timer == 170: self.net.send_action({'cmd': 'coin', 'result': self.coin_flip_result})
    ...
    def draw_room_prompt(self):
        ...
        draw_glass_rect(self.canvas, prompt_rect, HOUSE_BLUE, prompt_rect.h // 2)
    ...
    def draw_coin_face(self, surface, cx, cy, scale, is_red):
        r_w, r_h = int(120 * scale), 120
        for i in range(15): pygame.draw.ellipse(surface, (150, 110, 10), (cx - r_w, cy - r_h//2 + 15 - i, r_w * 2, r_h))
        pygame.draw.ellipse(surface, (255, 215, 0), (cx - r_w, cy - r_h//2, r_w * 2, r_h))
        pygame.draw.ellipse(surface, (220, 170, 20), (cx - int(r_w*0.85), cy - int(r_h*0.85)//2, int(r_w*0.85)*2, int(r_h*0.85)))
        pygame.draw.ellipse(surface, (255, 215, 0), (cx - int(r_w*0.75), cy - int(r_h*0.75)//2, int(r_w*0.75)*2, int(r_h*0.75)))
        
        if is_red: draw_maple_leaf(surface, cx, cy, 2.0 * scale, HOUSE_RED)
        else:
            rw = int(55 * scale)
            pygame.draw.ellipse(surface, (90, 95, 100), (cx - rw, cy - 10, rw*2, 30))
            pygame.draw.ellipse(surface, (150, 155, 160), (cx - rw, cy - 25, rw*2, 35))
            pygame.draw.ellipse(surface, HOUSE_BLUE, (cx - rw, cy - 15, rw*2, 18))
            pygame.draw.ellipse(surface, (170, 175, 180), (cx - rw*0.75, cy - 32, rw*1.5, 25))
    ...
    def draw_coin_toss_screen(self):
        ...
        if self.coin_timer > 40:
            is_red = (t // 15) % 2 == 0; text = "FLIPPING FOR HAMMER..."
        else:
            is_red = self.coin_flip_result == 0; text = "RED GETS HAMMER" if is_red else "YELLOW GETS HAMMER"
            
        if scale_x > 0.05: self.draw_coin_face(self.canvas, cx, cy, scale_x, is_red)
    ...
    def draw_ui(self):
        ...
        if self.game_mode == "CHALLENGE":
            ...
            self.canvas.blit(t1, (BASE_WIDTH//2 - t1.get_width()//2, 30)); self.canvas.blit(t2, (BASE_WIDTH//2 - t2.get_width()//2, 85))
        else:
            self.canvas.blit(self.font.render("RED", True, HOUSE_RED), (30, 20)); self.canvas.blit(self.font.render("YLW", True, TEAM_YELLOW), (30, 70))
            
            spacing = min(100, (BASE_WIDTH - 250) // self.total_ends_pref)
            for e in range(1, self.total_ends_pref + 1):
                cx = 120 + (e * spacing); self.canvas.blit(self.small_font.render(str(e), True, (140, 150, 165)), (cx, 12))
        ...
        if self.turn_state == "AIMING":
            ...
            draw_glass_rect(self.canvas, self.btn_curl_l, (215, 225, 240), 16, self.btn_curl_l.collidepoint(m_pos.x, m_pos.y))
            lbl_l = self.small_font.render("◀ CURL L", True, BLACK); self.canvas.blit(lbl_l, lbl_l.get_rect(center=self.btn_curl_l.center))
            
            draw_glass_rect(self.canvas, self.btn_curl_r, (215, 225, 240), 16, self.btn_curl_r.collidepoint(m_pos.x, m_pos.y))
            lbl_r = self.small_font.render("CURL R ▶", True, BLACK); self.canvas.blit(lbl_r, lbl_r.get_rect(center=self.btn_curl_r.center))

            if self.is_dragging:
                pull = pygame.math.Vector2(-self.virtual_pull.x, -self.virtual_pull.y)
                if pull.length() > 10:
                    spos, svel = pygame.math.Vector2(self.active_stone.pos), pull.normalize() * min(42.0, pull.length() / 8.0)
                    for i in range(140):
    ...
    def render(self):
        ww, wh = self.screen.get_size(); scale = min(ww / BASE_WIDTH, wh / BASE_HEIGHT); sw, sh = int(BASE_WIDTH * scale), int(BASE_HEIGHT * scale)
    ...
    def run(self):
        while True:
            for event in pygame.event.get():
                ...
                if event.type == MOUSEBUTTONDOWN and event.button == 1:
                    m_pos = self.scale_mouse(pygame.mouse.get_pos())
                    if self.app_state in ["MENU", "CHALLENGE_MENU", "ROOM_PROMPT"]:
